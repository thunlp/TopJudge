...
def check_law(data):
    ...
    if len(arr1) != 1:# or len(arr2) != 1:
        return False
    # print((arr1[0][0],arr1[0][1]) in law_dict1)
    # print(law_dict1)
...
def analyze_law1(data, config):
    ...
    for x, y, z in data:
        ...
        arr1.append((x, z))
    # print(data)
    # return law_dict1[(data[0], data[1])]
...
def generate_vector_art(data, config):
    # data = parse_sentence(data, config)
    vec = []
    for xx in data:
        vec_tmp = []
        for x in xx:
            ...
            if(len(temp_vec) > 128):
                print(x)
            vec_tmp.append(torch.stack(temp_vec))

        temp_vec = []
        while len(temp_vec) < config.getint("data", "sentence_len"):
            temp_vec.append(torch.from_numpy(transformer.load("BLANK")))

        while len(vec_tmp) < config.getint("data", "sentence_num"):
            vec_tmp.append(torch.stack(temp_vec))

        vec.append(torch.stack(vec_tmp))       
...
def parse(data, config):
    ...
    # print(data)
    # print(len(data["meta"]['top2law']))
    vector, len_vec = generate_vector(data["content"], config)
    vector_art = generate_vector_art(data["meta"]['top2law'], config)
    ...
    # print(label)
    # print(vector_art)
    return vector, vector_art, len_vec, torch.LongTensor(label)


def check(data, config):
    # print(data['meta']['crit'])
    data["meta"]["crit"] = list(data["meta"]["crit"])
