...
from docqa.selector import Selector
...
def get_selector(char_th: int, dim: int, answer_encoder, word_embed: FixedWordEmbedder):
    return Selector(
        encoder=DocumentAndQuestionEncoder(answer_encoder),
        ...
        reduce_seq=ReduceSequenceLayer("mean"),
        question_embed_layer=FullyConnected(dim, activation="relu"),
        content_embed_layer=FullyConnected(dim, activation="relu"),
        merge_layer=ConcatWithProduct(),
        merge_updater=ActivationLayer(activation="relu", bias=True),
        prob_generator=SequenceMapperSeq(VariationalDropoutLayer(0.8),
                                        FirstDimWeightedSum)
    )
...
def main():
    ...
    args.mode = "shared-norm"
    mode = args.mode
    out = model_dir.ModelDir('/home/yedeming/models/starter')

    if args.step is not None:
        if args.step == "latest":
            checkpoint = out.get_latest_checkpoint()
        else:
            checkpoint = out.get_checkpoint(int(args.step))
    else:
        checkpoint = out.get_best_weights()
    ...
    model = out.get_model()
    selctor_model = get_selector(100, 140, model.encoder, model.word_embed)
    ...
    evaluators = [LossEvaluator(), MultiParagraphSpanEvaluator(8, "triviaqa", mode != "merge")]
    ...
    train_params = get_triviaqa_train_params(n_epochs, n_dev, n_train)
    ...
    #trainer._train_async(model, data, checkpoint, "best", False,
    #       train_params, evaluators, out, notes, dry_run=True)

    trainer._train_selector_async(selctor, model, data, checkpoint, "best", False,
           train_params, evaluators, out, notes, dry_run=True)
